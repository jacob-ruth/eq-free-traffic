...
function aligned = alignMaxVel(data, n, position)
    ...
    fnVels = csape(n+1:1:2*n, data(n+1:2*n), 'periodic');   % interpolate and shift the velocities
    newpts = newpts + n;                                    % find the shift needed to move the max to the end
    aligned(n+1:2*n) = fnval(fnVels,newpts)';  
